...
- **Discharge planning** owns the six ToU minimum-SOC registers whenever the "Solcast: Prognose morgen" input is assigned: `f_soc` is derived every run from the cumulative net balance of up to three forecast days (`prognose_tage`, `b_stern_pct`) with the rule `max(target − B*, min(reserve, 100 − B*))`, clamped to the current SOC (hold) and the general minimum. It is written only when it can act (`tou_schreiben`). Nothing else writes the ToU registers; without the input they stay at the general minimum. The blueprint never charges from the grid: cell balancing is reached through PV by raising the planning target to 100 % when it is overdue.
- **Morning blockade and the throttled Prio 7 top-up exist for peak shaving only.** They run when `spitze_erwartet` is true: the highest forecast slot minus that slot's house load reaches 90 % of the peak-shaving threshold (`peak_erwartung_anteil`). Otherwise there is nothing to hold capacity for, `blockade_aktiv` is false and `amp_basis_13` is the maximum current.
