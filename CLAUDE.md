...
13. **Length check on every text you added.** Comment blocks max ~6 lines, input `description:` under 300 characters. Verify after writing, not at the next cleanup commit — the tidy-up of one release survived exactly four versions before the old style crept back in with the next new block.
