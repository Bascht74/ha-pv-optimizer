...
- `tools/optimizer_vergleich.py` — builds evcc-optimizer requests from recorded runs and prints its schedule next to the blueprint's values for the same run (`tests/test_optimizer_vergleich.py` covers the request and the reading of the answer). Development tool, not part of the blueprint.
