...
- **Discharge planning** owns the six ToU minimum-SOC registers whenever the "Solcast: Prognose morgen" input is assigned: `f_soc` is derived every run from the cumulative net balance of up to three forecast days (`prognose_tage`, `b_stern_pct`) with the rule `max(target − B*, min(reserve, 100 − B*))`, clamped to the current SOC (hold) and the general minimum. It is written only when it can act (`tou_schreiben`). Nothing else may write the ToU registers while it is active; the winter forced charge is gated on `not entlade_aktiv`. Without the input the registers stay at the general minimum.
